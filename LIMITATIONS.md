...
| 1.6 | **Interactive / RSP compute is only partly captured** — DAC/LSP cores are a fixed fraction of DRP; `developer_pilot.usdf_rsp_extra_fraction_of_drp_concurrent` adds a small concurrent bump. Actual notebook/portal usage varies. | Interactive capacity may still be under-provisioned. | Track RSP usage metrics and feed back into the model. |
| 1.8 | **DRP calendar overhead is parameterized, not measured** — `idle_and_inter_stage_multiplier` bundles idle between tasks and inter-stage evaluation (K-T: CM to refine). `developer_pilot.additional_node_days_per_year` is a placeholder until Yusra/CM supply a budget. | DRP concurrent cores can be wrong if multipliers are off. | Replace placeholders with CM-approved numbers. |
...
| 2.6 | **Compression is simplified** — Science/engineering raws use Key Numbers ratio; calibrations use Key Numbers ~2.3 GB/exposure vs logical frame (~0.281). Lossy output uses a single DRP-derived factor. | Uncertainty on coadd-related storage and future compression changes. | Update `imaging.calibration_compression` and per-product factors from ops data. |
| 2.8 | **`scratch_fraction` is a planning placeholder** — K-T: intermediates can dominate; ideally derive from Jim C. total vs release split in the DRP doc. | Scratch line in Ops Storage may be low or high vs reality. | Replace `storage_fractions.scratch_fraction` with fractions tied to release vs intermediate products. |
